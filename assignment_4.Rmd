...
author: |
  |zhengyi zhang
  | z.zhang9@students.uu.nl
  | https://github.com/justiceZZY1016/justice01
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
summary_data <- datasummary(growth + rgdp60 ~ treat * (Mean + median + SD + Min + Max),
             data = GrowthSW)
summary_data
...
the mean and the median of no revolution country are higher than countries with revolution
...
Use the data of 1960 as a benchmark to compare the development of subsequent years.Easier for statistics and forecasting
...
model2 <- lm(growth~rgdp60,data = GrowthSW)
model3 <- lm(growth~tradeshare,data = GrowthSW)
model4 <- lm(growth~education,data = GrowthSW)
```
...
list(model1, model2, model3, model4) |>
  ...
  row_spec(3:4, bold = TRUE, background = "red", color = "white") %>%
  print()
